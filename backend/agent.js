...
async function analyzeFile(client, content) {
  ...
  result.has_bug = result.has_bug || false;

  result.bug_description = result.bug_description || "";

  result.fixed_code = result.fixed_code || content;
...
export async function* runScan(folderPath) {
  ...
  if (files.length === 0) {
    yield {
      type: "log",
      message: "No supported code files found in this directory.",
    };

    yield {
      type: "complete",
      message: "Scan completed. Inspected 0 files, fixed 0 issues.",
    };
  ...
  for (const filePath of files) {
    ...
    try {
      ...
      yield {
        type: "error",
        message: `Could not read ${relPath}: ${error.message}`,
      };
    ...
    try {
      result = await analyzeFile(client, originalContent);
    } catch (error) {
      yield {
        type: "error",
        message: `LLM analysis failed for ${relPath}: ${error.message}`,
      };
    ...
    const description = result.bug_description || "Bug detected.";

    const fixedCode = result.fixed_code || originalContent;
    ...
    try {
      ...
      yield {
        type: "error",
        message: `Failed to patch ${relPath}: ${error.message}`,
      };
    ...
    yield {
      type: "patch_applied",
      file: relPath,
      message: `Applied patch (Backup created: ${path.basename(backupPath)})`,
    };
  }

  yield {
    type: "complete",
    message: `Scan completed. Inspected ${filesInspected} files, fixed ${bugsFixed} issues.`,
  };
}
